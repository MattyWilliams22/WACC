...
class RegisterMapping(val regMap: Map[Register, RegisterLocation]) {
  ...
  def mapInstructions(cfg: ControlFlowGraph, range: (Int, Int)): ListBuffer[Instruction] = {
    var newInstrs: ListBuffer[Instruction] = ListBuffer[Instruction]()
    for (node <- cfg.cfgNodes) {
      if (node.id >= range._1 && node.id <= range._2) {
        newInstrs ++= mapNode(node)
      }
    }
    newInstrs
  }

  private def mapNode(node: CFGNode): ListBuffer[Instruction] = {
    node.instr match {
      ...
      case Some(instr) => instr match {
        ...
        case RsbsInstr(reg1, reg2, operand) =>
          ...
          rsbsLines ++= after1
          rsbsLines
        case _ => ListBuffer(node.instr.get)
      }
    }
  }
}
